...
import { fmt, EXPENSE_CATS, INCOME_CATS, FREQUENCIES } from '../utils/format'
...
export default function RecurringPage() {
  ...
  const load=async()=>{setLoading(true);try{const{data}=await recurringApi.getAll();setRules(data.data)}catch{toast('Failed','error')}finally{setLoading(false)}}
  useEffect(()=>{load()},[])
  ...
  const save=async()=>{setSaving(true);try{const p={...form,endDate:form.endDate||null};modal==='create'?await recurringApi.create(p):await recurringApi.update(editing.id,p);toast('Saved','success');close();load()}catch(e){toast(e.response?.data?.message??'Failed','error')}finally{setSaving(false)}}
  const del=async id=>{if(!confirm('Deactivate?'))return;try{await recurringApi.delete(id);toast('Deactivated','success');load()}catch{toast('Failed','error')}}
  const cats=form.type==='INCOME'?INCOME_CATS:EXPENSE_CATS
  if(loading) return <div className="flex items-center justify-center h-64"><Spinner size="lg"/></div>
  return (
    <div className="space-y-4">
      <div className="flex justify-end"><button className="btn-primary" onClick={openCreate}><Plus className="w-4 h-4"/>New Rule</button></div>
      {!rules.length?<div className="card"><EmptyState icon={RefreshCw} title="No recurring rules" description="Automate salary, rent, subscriptions and more"/></div>:(
            ...
            <tbody>
              {rules.map(r=>(
                <tr key={r.id} className="table-row">
                  ...
                  <td className="table-cell text-xs text-slate-500">{fmt.date(r.nextExecutionDate)}</td>
      ...
      <Modal open={!!modal} onClose={close} title={modal==='create'?'New Recurring Rule':'Edit Rule'}>
        <div className="space-y-4">
          <div><label className="label">Name</label><input className="input" placeholder="Monthly Rent" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))}/></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="label">Type</label><select className="input" value={form.type} onChange={e=>setForm(f=>({...f,type:e.target.value,category:e.target.value==='INCOME'?'SALARY':'FOOD'}))}><option>INCOME</option><option>EXPENSE</option></select></div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className="label">Amount (Rs.)</label><input className="input" type="number" min="0.01" value={form.amount} onChange={e=>setForm(f=>({...f,amount:e.target.value}))}/></div>
            <div><label className="label">Frequency</label><select className="input" value={form.frequency} onChange={e=>setForm(f=>({...f,frequency:e.target.value}))}>{FREQUENCIES.map(f=><option key={f}>{f}</option>)}</select></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><label className="label">End Date (opt.)</label><input className="input" type="date" value={form.endDate} onChange={e=>setForm(f=>({...f,endDate:e.target.value}))}/></div>
          </div>
