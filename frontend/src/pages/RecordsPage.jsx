...
import { fmt, EXPENSE_CATS, INCOME_CATS } from '../utils/format'
...
export default function RecordsPage() {
  ...
  const [filters, setFilters] = useState({ type:'', keyword:'', dateFrom:'', dateTo:'' })
  ...
  const save = async () => {
    setSaving(true)
    try {
      modal==='create' ? await recordsApi.create(form) : await recordsApi.update(editing.id, form)
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-48">
          ...
          <input className="input pl-8" placeholder="Search description…" value={filters.keyword} onChange={e=>setFilters(f=>({...f,keyword:e.target.value}))}/>
        </div>
      ...
      <div className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-surface-border">
              <tr>{['Date','Type','Category','Amount','Description',''].map(h=><th key={h} className="table-header">{h}</th>)}</tr>
            </thead>
            <tbody>
              {loading ? <tr><td colSpan={6} className="py-16 text-center"><Spinner className="mx-auto"/></td></tr>
              : !records.length ? <tr><td colSpan={6}><EmptyState icon={ArrowLeftRight} title="No records found"/></td></tr>
              : records.map(r=>(
                <tr key={r.id} className="table-row">
                  ...
                  <td className="table-cell text-xs">{r.category.replace(/_/g,' ')}</td>
        ...
        {total>15 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-surface-border">
            <p className="text-xs text-slate-500">{page*15+1}–{Math.min((page+1)*15,total)} of {total}</p>
      ...
      <Modal open={!!modal} onClose={closeModal} title={modal==='create'?'Add Record':'Edit Record'}>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div><label className="label">Type</label>
              <select className="input" value={form.type} onChange={e=>setForm(f=>({...f,type:e.target.value,category:e.target.value==='INCOME'?'SALARY':'FOOD'}))}>
                <option>INCOME</option><option>EXPENSE</option></select></div>
            <div><label className="label">Category</label>
              <select className="input" value={form.category} onChange={e=>setForm(f=>({...f,category:e.target.value}))}>
                {cats.map(c=><option key={c}>{c}</option>)}</select></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="label">Amount (Rs.)</label><input className="input" type="number" min="0.01" step="0.01" value={form.amount} onChange={e=>setForm(f=>({...f,amount:e.target.value}))}/></div>
          ...
          <div><label className="label">Description</label><input className="input" placeholder="e.g. Monthly groceries" value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))}/></div>
