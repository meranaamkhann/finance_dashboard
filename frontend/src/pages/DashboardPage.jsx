  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setSummary(s.data.data); setTrend(t.data.data)
              ...
              <div key={i} className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="flex justify-between mb-1">
                    <span className="text-xs font-medium text-slate-700">{c.category.replace(/_/g,' ')}</span>
                    <span className="text-xs font-semibold">{fmt.currency(c.amount)}</span>
                  </div>
