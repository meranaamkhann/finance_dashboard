...
export default function Sidebar() {
  ...
  return (
    ...
    >
      <button
        onClick={() => nav('/')}
        className="px-5 py-5 border-b flex items-center gap-2.5 hover:opacity-90 transition-opacity"
      ...
      <div className="px-3 py-3 border-t" style={{ borderColor: 'var(--border)' }}>
        <div className="flex items-center gap-3 px-3 py-2 mb-1">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold truncate" style={{ color: 'var(--text-main)' }}>{user?.fullName}</p>
            <p className="text-xs" style={{ color: 'var(--text-faint)' }}>{user?.role}</p>
          </div>
        </div>
        <button
          onClick={logout}
          className="flex items-center gap-3 w-full px-3 py-2 text-sm font-medium rounded-lg transition-colors hover:text-red-500"
