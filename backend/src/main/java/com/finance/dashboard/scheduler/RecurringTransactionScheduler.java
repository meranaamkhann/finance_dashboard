...
public class RecurringTransactionScheduler {
    ...
    public void execute() {
        ...
        for (RecurringTransaction rt : due) {
            try { post(rt, today); }
                ...
                log.error("Failed recurring [id={}]: {}", rt.getId(), e.getMessage());
            }
        }
    }
    ...
    private void post(RecurringTransaction rt, LocalDate today) {
        FinancialRecord r = FinancialRecord.builder()
                .type(rt.getType()).category(rt.getCategory().name()).amount(rt.getAmount())
                .date(today).description("Auto: " + rt.getName())
        ...
        notificationService.send(rt.getUser(), NotificationType.RECURRING_EXECUTED,
                "Auto-posted: " + rt.getName() + " — Rs." + rt.getAmount());
        if (rt.getType() == TransactionType.EXPENSE)
            budgetAlertService.evaluate(rt.getUser(), rt.getCategory(), today);
        auditService.log(AuditAction.RECURRING_EXECUTED, "SYSTEM",
                "RecurringTransaction", rt.getId(), null, null, null, "Auto-posted: " + rt.getName());
    }
}
