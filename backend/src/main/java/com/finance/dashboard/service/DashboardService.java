...
public class DashboardService {
    ...
    public DashboardSummaryResponse getSummary() {
        ...
        BigDecimal sr  = income.compareTo(BigDecimal.ZERO) > 0
                ...
                : BigDecimal.ZERO;
        return DashboardSummaryResponse.builder().totalIncome(income).totalExpense(expense)
                .netBalance(net).savingsRate(sr)
                ...
                .topCategories(getCategoryBreakdown(uid, som, today).stream().limit(5).toList())
                .build();
    }
    ...
    private List<CategorySummaryResponse> getCategoryBreakdown(Long uid, LocalDate from, LocalDate to) {
        ...
        BigDecimal total = rows.stream().map(r -> new BigDecimal(r[1].toString()))
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        return rows.stream().map(r -> {
            BigDecimal amt = new BigDecimal(r[1].toString());
            double pct = total.compareTo(BigDecimal.ZERO) > 0
                    ? amt.divide(total, 4, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(100)).doubleValue() : 0;
            return CategorySummaryResponse.builder().category(Category.valueOf(r[0].toString()))
                    .amount(amt).percentage(Math.round(pct * 100.0) / 100.0).build();
        }).toList();
    }
    ...
    public List<MonthlyTrendResponse> getMonthlyTrend(int months) {
        ...
        return recordRepository.monthlyTrendByUser(uid, from, LocalDate.now()).stream().map(r -> {
    ...
    public List<WeeklyTrendResponse> getWeeklyTrend(int weeks) {
        Long uid = securityUtils.getCurrentUserId();
        LocalDate from = LocalDate.now().minusWeeks(weeks), to = LocalDate.now();
        return recordRepository.weeklyTrendByUser(uid, from, to).stream().map(r -> {
            int year = ((Number) r[0]).intValue(), week = ((Number) r[1]).intValue();
            BigDecimal inc = r[2] != null ? new BigDecimal(r[2].toString()) : BigDecimal.ZERO;
            BigDecimal exp = r[3] != null ? new BigDecimal(r[3].toString()) : BigDecimal.ZERO;
            return WeeklyTrendResponse.builder().year(year).weekOfYear(week)
    ...
    public Map<String, BigDecimal> getSpendingByDayOfWeek(LocalDate from, LocalDate to) {
        validateRange(from, to);
        Long uid = securityUtils.getCurrentUserId();
        // Hibernate 6 extract(day of week from x) returns 1=Sun..7=Sat
